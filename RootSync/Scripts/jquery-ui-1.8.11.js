...
$.extend(Datepicker.prototype, {
	...
	_dialogDatepicker: function(input, date, onSelect, settings, pos) {
		var inst = this._diasigninst; // internal instance
		if (!inst) {
			this.uuid += 1;
			var id = 'dp' + this.uuid;
			this._diasigninput = $('<input type="text" id="' + id +
				'" style="position: absolute; top: -100px; width: 0px; z-index: -10;"/>');
			this._diasigninput.keydown(this._doKeyDown);
			$('body').append(this._diasigninput);
			inst = this._diasigninst = this._newInst(this._diasigninput, false);
			inst.settings = {};
			$.data(this._diasigninput[0], PROP_NAME, inst);
		}
		extendRemove(inst.settings, settings || {});
		date = (date && date.constructor == Date ? this._formatDate(inst, date) : date);
		this._diasigninput.val(date);
		...
		this._diasigninput.css('left', (this._pos[0] + 20) + 'px').css('top', this._pos[1] + 'px');
		inst.settings.onSelect = onSelect;
		this._inDialog = true;
		this.dpDiv.addClass(this._dialogClass);
		this._showDatepicker(this._diasigninput[0]);
		if ($.blockUI)
			$.blockUI(this.dpDiv);
		$.data(this._diasigninput[0], PROP_NAME, inst);
		return this;
	...
	_hideDatepicker: function(input) {
		...
		if (this._datepickerShowing) {
			...
			if (this._inDialog) {
				this._diasigninput.css({ position: 'absolute', left: '0', top: '-100px' });
				if ($.blockUI) {
					$.unblockUI();
